...
Track tasks, organize work, and keep projects moving forward. This is a native tldw_server persona archetype: a first-run
...
- [project_manager.yaml](project_manager.yaml): original YAML, including persona, module, policy,
...
The archetype seeds enabled MCP modules: kanban, notes, filesystem, knowledge. Suggested external servers:
github, slack. These are configuration suggestions, not bundled services or automatic
...
item_dir = Path("/path/to/tldw-stuff/personas/project-manager")
loaded = load_archetypes_from_directory(item_dir)
assert set(loaded) == {"project_manager"}
assert get_archetype("project_manager") == loaded["project_manager"]
assert list_archetypes()[0].key == "project_manager"
print(loaded["project_manager"].persona.name)
```
...
startup reads `tldw_Server_API/Config_Files/persona_archetypes/project_manager.yaml`. To restore
that default, back up any customized file, copy this YAML to that location, and
restart the server. Its key is `project_manager`. For a separate custom archetype, use a
...
Input: “Break a documentation refresh into tasks.”

Intended prompt behavior: An action-oriented plan with concrete next steps and potential blockers. This is an illustrative expectation, not a
recorded model response. A loader check prints `Project Manager`.
...
The loader returned exactly `project_manager`; its persona name and prompt were nonempty;
