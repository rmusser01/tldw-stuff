...
Start from scratch and configure everything yourself. This is a native tldw_server persona archetype: a first-run
...
- [blank_canvas.yaml](blank_canvas.yaml): original YAML, including persona, module, policy,
...
item_dir = Path("/path/to/tldw-stuff/personas/blank-canvas")
loaded = load_archetypes_from_directory(item_dir)
assert set(loaded) == {"blank_canvas"}
assert get_archetype("blank_canvas") == loaded["blank_canvas"]
assert list_archetypes()[0].key == "blank_canvas"
print(loaded["blank_canvas"].persona.name)
```
...
startup reads `tldw_Server_API/Config_Files/persona_archetypes/blank_canvas.yaml`. To restore
that default, back up any customized file, copy this YAML to that location, and
restart the server. Its key is `blank_canvas`. For a separate custom archetype, use a
...
Intended prompt behavior: A general assistant response; no specialized traits, MCP modules, or starter commands are seeded. This is an illustrative expectation, not a
recorded model response. A loader check prints `Assistant`.
...
The loader returned exactly `blank_canvas`; its persona name and prompt were nonempty;
