...
Deep-dive into any topic with structured analysis and source management. This is a native tldw_server persona archetype: a first-run
...
| Source | [Pinned upstream YAML](https://github.com/rmusser01/tldw_server/blob/36b846628d7755e91e0d5539740a9f5c9a837966/tldw_Server_API/Config_Files/persona_archetypes/research_assistant.yaml) |
...
- [research_assistant.yaml](research_assistant.yaml): original YAML, including persona, module, policy,
...
The archetype seeds enabled MCP modules: media, knowledge, notes. Suggested external servers:
arxiv, github, zotero. These are configuration suggestions, not bundled services or automatic
...
item_dir = Path("/path/to/tldw-stuff/personas/research-assistant")
loaded = load_archetypes_from_directory(item_dir)
assert set(loaded) == {"research_assistant"}
assert get_archetype("research_assistant") == loaded["research_assistant"]
assert list_archetypes()[0].key == "research_assistant"
print(loaded["research_assistant"].persona.name)
```
...
startup reads `tldw_Server_API/Config_Files/persona_archetypes/research_assistant.yaml`. To restore
that default, back up any customized file, copy this YAML to that location, and
restart the server. Its key is `research_assistant`. For a separate custom archetype, use a
...
Input: “Compare these two ingested papers.”

Intended prompt behavior: A structured comparison that cites the ingested sources and flags uncertainty. This is an illustrative expectation, not a
recorded model response. A loader check prints `Research Assistant`.
...
The loader returned exactly `research_assistant`; its persona name and prompt were nonempty;
