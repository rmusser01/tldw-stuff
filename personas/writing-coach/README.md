...
Sharpen your prose with thoughtful feedback and structured guidance. This is a native tldw_server persona archetype: a first-run
...
| Source | [Pinned upstream YAML](https://github.com/rmusser01/tldw_server/blob/36b846628d7755e91e0d5539740a9f5c9a837966/tldw_Server_API/Config_Files/persona_archetypes/writing_coach.yaml) |
...
- [writing_coach.yaml](writing_coach.yaml): original YAML, including persona, module, policy,
...
The archetype seeds enabled MCP modules: notes, media. Suggested external servers:
...
item_dir = Path("/path/to/tldw-stuff/personas/writing-coach")
loaded = load_archetypes_from_directory(item_dir)
assert set(loaded) == {"writing_coach"}
assert get_archetype("writing_coach") == loaded["writing_coach"]
assert list_archetypes()[0].key == "writing_coach"
print(loaded["writing_coach"].persona.name)
```
...
startup reads `tldw_Server_API/Config_Files/persona_archetypes/writing_coach.yaml`. To restore
that default, back up any customized file, copy this YAML to that location, and
restart the server. Its key is `writing_coach`. For a separate custom archetype, use a
...
Input: “Review this paragraph for clarity.”

Intended prompt behavior: Constructive feedback on strengths, structure, and wording. This is an illustrative expectation, not a
recorded model response. A loader check prints `Writing Coach`.
...
The loader returned exactly `writing_coach`; its persona name and prompt were nonempty;
