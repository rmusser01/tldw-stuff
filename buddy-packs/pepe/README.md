...
```sh
python scripts/build_buddy_collection.py buddy-packs/pepe
python tests/check_buddy_collection.py --host chatbook --pack pepe
python tests/check_buddy_collection.py --host server --pack pepe
```
...
Pepe the Frog is **Matt Furie's** character from Boy's Club, as documented by
[his publisher, Fantagraphics](https://www.fantagraphics.com/collections/matt-furie).
This is an unofficial adaptation; no existing Pepe illustrations are bundled.
The pack does not license the underlying character or Matt Furie's artwork.
