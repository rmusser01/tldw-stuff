...
A white pelican with a long orange bill and throat pouch, riding a small teal bicycle.

The pelican stays on the bicycle while it blinks, speaks, listens, waves and reacts. The bicycle base, feet and wheel positions remain fixed; the animations use its face, bill and wings.

## Preview and download

<img src="preview.png" alt="Pelican on a Bicycle still preview" width="192" height="192"> <img src="preview.gif" alt="Pelican on a Bicycle animated expression preview" width="192" height="192">

[**Download Pelican on a Bicycle**](pelican-bicycle.tldw-persona-vpack) · 169,949 bytes · [SHA-256 checksums](SHA256SUMS)
...
The generated alpha was preserved, with very faint stray pixels removed, including inside the bicycle frame and wheel openings. All poses use one scale and a shared tire baseline. The idle-frame bicycle base and lower body are reused across the sheet to remove wheel and pedal drift.
...
```sh
python scripts/build_buddy_collection.py buddy-packs/pelican-bicycle
python tests/check_buddy_collection.py --host chatbook --pack pelican-bicycle
python tests/check_buddy_collection.py --host server --pack pelican-bicycle
```
