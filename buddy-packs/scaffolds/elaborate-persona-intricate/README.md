# Elaborate Persona Intricate — scaffold

Catalog scaffold fixture, not final character art or animation: High-detail fantasy/sci-fi starter demonstrating multiple custom-state rows.
...
| Production status | `scaffold`; tier `intricate` |
...
- [elaborate-persona-intricate.tldw-persona-vpack](elaborate-persona-intricate.tldw-persona-vpack): native portable archive, 5,738 bytes, containing 7 PNG assets and 7 state mappings.
...
States: `error`, `idle`, `listening`, `mood.focused`, `speaking`, `thinking`, `tool.media_import`.

## Verification

Native server export, preview and committed import succeeded, producing a
new draft with 7 matching asset checksums and 7 states.
Native Chatbook import, publication to a temporary profile/SQLite database,
and active-graph reload also succeeded with 7 assets.
