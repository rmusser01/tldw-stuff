...
Create, edit and analyze Excel .xlsx workbooks from supplied tables or Chatbook material, preserving formulas and distinguishing calculated results from cached values.

Example request: “Build a scenario workbook from these line items with editable quantities, unit costs, totals and a source column. Keep unknown costs blank.”
...
openpyxl**. No account, network call or other skill is required to create
...
The [example](scripts/create_workbook.py) is a small runnable starting point with
...
```bash
python scripts/create_workbook.py example.xlsx
```
...
The existing [Nous Research Hermes xlsx pack](../../hermes/xlsx/README.md)
