...
with complete attribution and pinned source snapshots. All eleven were checked
with Chatbook’s native directory importer. Ten preserved every file; skill-creator
imported with one empty Python package marker omitted (see its README). The collection remains
...
Each item includes its original SKILL.md, supporting files, license text,
source revision, author/publisher attribution, and checksums. Source folders
are grouped by publisher; select an individual skill directory when importing.
Preserved instructions may refer to the upstream host’s tools or installation
paths. Consult the item README before running them.
